...
```
Error: locator.fill: Error: strict mode violation: locator('iframe#pact') resolved to 2 elements:
...
Call log:
  - waiting for locator('iframe#pact').contentFrame().locator('#training').first()
...
```ts
  ...
  327 |     const sd:FrameLocator=page.frameLocator('iframe#pact');
  328 |    // sd.locator('#tea').first().fill("Yes");
> 329 |     sd.locator("#training").first().fill("No");
      |                                     ^ Error: locator.fill: Error: strict mode violation: locator('iframe#pact') resolved to 2 elements:
  330 | 
  331 | 
  332 | });
